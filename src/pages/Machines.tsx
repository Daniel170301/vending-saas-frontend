...
const Machines = () => {
  ...
  // Cargar inventario del modal (Ojito)
  useEffect(() => {
    if (!viewing) {
      setProductosModal([]);
      return;
  ...
  // FUNCIÓN PARA GUARDAR LA CAPACIDAD MANUALMENTE
  const ejecutarGuardarCapacidad = async (codigoMotor: string, nuevaCapacidad: number, producto: any) => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(`Capacidad de R${codigoMotor} guardada con éxito`);
  ...
  return (
    <div className="container py-8">
      ...
      {/* DIALOG DE CREACIÓN Y EDICIÓN COMPLETO CON SUS TABS ORIGINALES */}
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex gap-2 border-b pb-2 mb-4">
            <Button type="button" variant={tab === "data" ? "default" : "ghost"} size="sm" onClick={() => setTab("data")}>Datos Generales</Button>
            <Button type="button" variant={tab === "layout" ? "default" : "ghost"} size="sm" onClick={() => setTab("layout")}>Diseño de Bandejas</Button>
          </div>
            ...
            <div className="space-y-5">
              <div>
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <Label>Código *</Label>
                    <Input value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} placeholder="Ej: D4-8A..." disabled={!!editing} />
                  </div>
                </div>
                <div className="mt-3">
                  <Label>Ubicación / Dirección</Label>
                  <Input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} placeholder="Ej: Av. Central 123" />
                </div>
              </div>

              <div className="grid sm:grid-cols-3 gap-3 border-t pt-3">
                <div>
                  <Label>Marca Máquina</Label>
                  <Input value={form.brand} onChange={(e) => setForm({ ...form, brand: e.target.value })} placeholder="Ej: Vendo" />
                </div>
                <div>
                  <Label>Modelo Máquina</Label>
                  <Input value={form.model} onChange={(e) => setForm({ ...form, model: e.target.value })} placeholder="Ej: VDI 800" />
                </div>
                <div>
                  <Label>Placa / Serie</Label>
                  <Input value={form.plate} onChange={(e) => setForm({ ...form, plate: e.target.value })} placeholder="Ej: SN-99812" />
                </div>
              </div>
              ...
              <div className="border-t pt-3">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-semibold text-sm text-primary flex items-center gap-1"><Coins className="h-4 w-4" /> Configuración de Monedero</h4>
                  <input type="checkbox" id="coin_enabled" checked={form.coin_enabled} onChange={(e) => setForm({ ...form, coin_enabled: e.target.checked })} className="rounded border-gray-300 text-primary focus:ring-primary h-4 w-4" />
                </div>
                {form.coin_enabled && (
                  <div className="grid sm:grid-cols-3 gap-3 bg-slate-50 p-3 rounded-xl">
                    <div>
                      <Label>Base de Monedas ($)</Label>
                      <Input type="number" value={form.coin_base} onChange={(e) => setForm({ ...form, coin_base: e.target.value })} placeholder="0.00" />
                    </div>
                    <div>
                      <Label>Marca Monedero</Label>
                      <Input value={form.coin_brand} onChange={(e) => setForm({ ...form, coin_brand: e.target.value })} placeholder="Ej: Mei" />
                    </div>
                    <div>
                      <Label>Placa Monedero</Label>
                      <Input value={form.coin_plate} onChange={(e) => setForm({ ...form, coin_plate: e.target.value })} placeholder="Ej: CF7000" />
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="border-t pt-3">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-semibold text-sm text-primary flex items-center gap-1"><Banknote className="h-4 w-4" /> Configuración de Billetero</h4>
                  <input type="checkbox" id="bill_enabled" checked={form.bill_enabled} onChange={(e) => setForm({ ...form, bill_enabled: e.target.checked })} className="rounded border-gray-300 text-primary focus:ring-primary h-4 w-4" />
                </div>
                {form.bill_enabled && (
                  <div className="grid sm:grid-cols-3 gap-3 bg-slate-50 p-3 rounded-xl">
                    <div>
                      <Label>Marca Billetero</Label>
                      <Input value={form.bill_brand} onChange={(e) => setForm({ ...form, bill_brand: e.target.value })} placeholder="Ej: ICT" />
                    </div>
                    <div>
                      <Label>Modelo Billetero</Label>
                      <Input value={form.bill_model} onChange={(e) => setForm({ ...form, bill_model: e.target.value })} placeholder="Ej: P70" />
                    </div>
                    <div>
                      <Label>Placa Billetero</Label>
                      <Input value={form.bill_plate} onChange={(e) => setForm({ ...form, bill_plate: e.target.value })} placeholder="Ej: SN-BILLE" />
                    </div>
                  </div>
                )}
              </div>
          ...
          {tab === "layout" && (
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                <h4 className="font-semibold text-sm text-primary">Estructura de Bandejas</h4>
                <Button type="button" variant="outline" size="sm" onClick={() => setLayout({ ...layout, trays: [...layout.trays, newTray(layout.trays.length)] })}><Plus className="h-4 w-4 mr-1" /> Agregar Bandeja</Button>
              </div>
              <div className="space-y-4 max-h-[50vh] overflow-y-auto pr-2">
                {layout.trays.map((t, ti) => (
                  <Card key={t.id} className="p-4 bg-slate-50/50">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <LayoutGrid className="h-4 w-4 text-muted-foreground" />
                        <Input size={30} className="h-8 font-medium w-40" value={t.label} onChange={(e) => {
                          const nt = [...layout.trays];
                          nt[ti].label = e.target.value;
                          setLayout({ ...layout, trays: nt });
                        }} />
                      </div>
                      <Button type="button" variant="ghost" size="icon" className="text-destructive h-8 w-8" onClick={() => setLayout({ ...layout, trays: layout.trays.filter((_, i) => i !== ti) })}><Trash2 className="h-4 w-4" /></Button>
                    </div>
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">
                      {t.springs.map((s, si) => (
                        <div key={s.id} className="bg-white p-2 rounded-lg border text-center relative group">
                          <span className="text-[10px] font-bold text-muted-foreground block mb-1">{s.label}</span>
                          <div className="space-y-1">
                            <Input type="number" placeholder="Cap" className="h-7 text-xs text-center px-1" value={s.capacity || ""} onChange={(e) => {
                              const nt = [...layout.trays];
                              nt[ti].springs[si].capacity = Number(e.target.value);
                              setLayout({ ...layout, trays: nt });
                            }} />
                            <Input type="number" placeholder="$ Precio" className="h-7 text-xs text-center px-1" value={s.sale_price || ""} onChange={(e) => {
                              const nt = [...layout.trays];
                              nt[ti].springs[si].sale_price = Number(e.target.value);
                              setLayout({ ...layout, trays: nt });
                            }} />
                          </div>
                        </div>
              ...
              <Button variant="hero" className="w-full mt-4" onClick={save}>Guardar Máquina y Layout</Button>
            </div>
          )}
        </DialogContent>
      </Dialog>

      {/* DIALOG DEL OJITO: DETALLE DE RESORTES (GUARDADO MANUAL CORREGIDO) */}
                ...
                <div key={numBandeja} className="bg-card rounded-2xl border shadow-sm p-5">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                    {[0, 1, 2, 3, 4, 5].map((posicion) => {
                      ...
                      return (
                        <div key={codigoMotor} className="border-2 border-dashed rounded-xl p-3 flex flex-col items-center justify-center min-h-[140px] relative bg-white hover:bg-accent/5 transition-colors">
